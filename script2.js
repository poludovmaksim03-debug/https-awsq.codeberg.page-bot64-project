class HomeworkCheckerBot {
    ...
    async callChatGPT(prompt) {
        try {
            this.status.textContent = 'Обращение к AI...';
            ...
            console.error('Ошибка вызова ChatGPT:', error);
            this.status.textContent = 'Ошибка получения ответа от AI';
            throw error;
        }
    }
    ...
    startAutoScanning() {
        ...
        this.autoScanInterval = setInterval(async () => {
            ...
            this.addChatMessage(`🧠 Проверка от AI:\n${solution}`, false);
                } catch (error) {
